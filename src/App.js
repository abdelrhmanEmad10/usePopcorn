      ...
      async function fetchMovies() {
        try {
          ...
          const res = await fetch(
            `http://www.omdbapi.com/?apikey=c2d220a1&s=${query}`
          );
          if (!res.ok)
            throw new Error("Something wrong happened while fetching");
          const data = await res.json();
          if (data.Search === undefined) throw new Error("No movies found");
  ...
  useEffect(
    function () {
      async function getMovieData() {
        setIsLoading(true);
        const res = await fetch(
          `http://www.omdbapi.com/?apikey=c2d220a1&i=${selectedMovie}`
        );
        const data = await res.json();
        setMovie(data);
        setIsLoading(false);
  ...
  return (
    <div className="details">
      {isLoading ? (
        ...
        <>
          <header>
            <button onClick={handleClearMovie} className="btn-back">
              &larr;
            </button>
...
function WatchedList({ watched, handleDeleteMovie }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie handleDeleteMovie={handleDeleteMovie} movie={movie} />
      ))}
    </ul>
  );
}
